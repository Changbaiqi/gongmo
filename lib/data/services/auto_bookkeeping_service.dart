...
class AutoBookkeepingService {
  ...
  static (FinanceType, double, String?)? parseMeituan(String text) {
    ...
    // 营销/广告通知一律不记账：美团推广大量使用这些词，
    // 且文案里的 ￥ 金额是商品价格而不是支付金额
    if (RegExp(
            r'红包|优惠券|代金券|消费券|券包|立减|返现|免单|抽奖|待领取|领取|限时|抢购|折扣|特价|活动|福利|补贴|会员|开通|续费|过期|到期|邀请|推荐|下载|签到|奖励|任务|升级|专享|优惠')
        .hasMatch(t)) {
      return null;
    }
    ...
    // 支出：必须有明确的支付措辞，不能只凭 ￥ 金额
    // （订单金额、商品价格等文案也带 ￥，会被误记账）
    for (final re in [
      RegExp(
  ...
  static (FinanceType, double, String?)? parseWechat(String text) {
    ...
    // 收入：微信支付收款12.34元 / 已收款￥24.81 / 个人收款码到账￥5.00
    for (final re in [
      RegExp(
          r'(?:已收款|收款到账|收款成功|已到账|到账|入账)\s*[￥¥]?\s*([0-9]+(?:\.[0-9]+)?)'),
      RegExp(r'收款\s*([0-9]+(?:\.[0-9]+)?)\s*元'),
